...
def retrieve_data():
    ...
    for i in range(len(result_data)):
        if(i%2!=0):
            table.insert(parent='', index='end', iid=i, text="Label", values=(result_data[i][0],result_data[i][1],result_data[i][2]),tag = 'odd')
        else:
            table.insert(parent='', index='end', iid=i, text="Label", values=(result_data[i][0],result_data[i][1],result_data[i][2]))

def add():
    ep.edit_datatable(columns,table,'add')
...
def select_data(event):
    global table
    ep.edit_datatable(columns,table,'edit')
...
def create_table_GUI(main,text,columns,width,double_click):
    ...
    fr_table = tk.Frame(main,width=500,height=100)
...
def create_main_window():
    ...
    main = tk.Tk()
    main.title("Positions | Employee Payroll Management System")
    heading = tkfont.Font(family="Times New Roman", size=25,weight='bold')
    text = tkfont.Font(family="Times New Roman", size=16)
    tk.Label(main,text = "LIST OF POSITIONS",font=heading).pack(anchor='n')
    columns = ('id','Position Name','Department')
    width = [50,500,300]
