'''add/update/delete attendance log'''
...
def add(edit_datatable, table, columns):
    global entry,c
    create_connection()
    sql = '''INSERT INTO attendance(emp_id, month_, year_, total_working_days, emp_working_days)
                VALUES(:b,:c,:d,:e,:f)'''
    try:
         result = c.execute(sql,b=emp_id,c=month,d=year,e=entry[4].get(),f=entry[5].get())
    except oracle.IntegrityError:
        messagebox.showerror(message='All fields are mandatory')
    except oracle.DatabaseError:
        messagebox.showerror(message='Enter only numeric values')
    else:
        conn.commit()
        messagebox.showinfo("Success","Successfully added attendance log")
...
def update(edit_datatable,table,columns,selected_row):
    ...
    try:
        ...
        messagebox.showerror(message='All fields are mandatory. Enter only numeric values')
    else:
        conn.commit()
        messagebox.showinfo("Success","Successfully updated attendance log")
        edit_datatable.destroy()
    close_connection()
...
def delete(edit_datatable,table,columns,selected_row):
    ...
    conn.commit()
    messagebox.showinfo("Success","Successfully deleted attendance log")
    close_connection()
    edit_datatable.destroy()
...
def edit_datatable(columns,table,command): 
    ...
    edit_datatable.geometry('500x265')
    if(command=='add'):
        edit_datatable.title("Add Attendance Log | Employee Payroll Management System")
        tk.Label(edit_datatable,text = "ADD ATTENDANCE LOG",font=heading).pack(side='top',anchor='center')
    else:
        edit_datatable.title("Edit Attendance Log | Employee Payroll Management System")
        tk.Label(edit_datatable,text = "EDIT ATTENDANCE LOG",font=heading).pack(side='top',anchor='center')
