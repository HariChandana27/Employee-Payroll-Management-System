...
import os
import edit_emp_allowances as eea
...
def add():
    eea.edit_datatable(columns,table,'add')
...
def select_data(event):
    global table
    eea.edit_datatable(columns,table,'edit')
...
def create_main_window():
    ...
    main = tk.Tk()
    main.title("Employee Allowances | Employee Payroll Management System")
    heading = tkfont.Font(family="Times New Roman", size=25,weight='bold')
    text = tkfont.Font(family="Times New Roman", size=16)
    tk.Label(main,text = "LIST OF EMPLOYEE ALLOWANCES",font=heading).pack(anchor='n')
    columns = ('ref no','Employee','Month','Year','Allowance Name','Amount')
    width = [75,150,85,85,300,150]
