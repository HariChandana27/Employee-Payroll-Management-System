'''add/update/delete emploloyee allowances' data'''
...
def add(edit_datatable, table, columns):
    global entry,c
    create_connection()
    sql = '''INSERT INTO emp_allowances(emp_id, month_, year_, allowance_name, amount)
                 VALUES(:a,:b,:c,:d,:e)'''
    try:
        ...
        messagebox.showinfo("Success","Successfully added employee's allowance")
...
def update(edit_datatable,table,columns,selected_row):
    ...
    try:
        ...
        messagebox.showinfo("Success","Successfully updated department's allowance")
        edit_datatable.destroy()
    conn.commit()
    close_connection()
...
def delete(edit_datatable,table,columns,selected_row):
    global entry,c
    create_connection()
    sql = '''DELETE FROM emp_allowances
                 WHERE ref_no=:a'''
