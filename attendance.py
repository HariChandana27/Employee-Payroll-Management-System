...
def add():
    ea.edit_datatable(columns,table,'add')
...
def select_data(event):
    global table
    ea.edit_datatable(columns,table,'edit')
...
def create_main_window():
    ...
    main = tk.Tk()
    main.title("Attendance log | Employee Payroll Management System")
    heading = tkfont.Font(family="Times New Roman", size=25,weight='bold')
    text = tkfont.Font(family="Times New Roman", size=16)
    tk.Label(main,text = "ATTENDANCE LOG",font=heading).pack(anchor='n')
    columns = ('ref no','Employee ID','Month','Year','Total working days','Employee working days')
    width = [75,160,90,85,200,200]
